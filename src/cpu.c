...
// -----------------------------------------------------------------------------
// this portion of code has nothing to do with the internal workings of a Z80
//  and is here only as an aid to fast loading/saving of tape files

int TAPE_FastLoad(BYTE, WORD, WORD);
void TAPE_FastSave(WORD, WORD);

void ROM_LdBytes()
{
    if ((!CPU_M1) || (cpuAddress != 0x0559 && cpuAddress != 0x056c))
    {
        return;
    }

    // AF is exchanged at rom address 0557h
    //  so it must be exchanged back like at address 05a9h
    regFF[0] ^= 1; regAFu = &regMain[regFF[0]];

    if (TAPE_FastLoad(regA, regIX, regDE) == 1)
    {
        regF |= CF;
    }
    else
...
void ROM_SaContrl()
{
    if ((!CPU_M1) || cpuAddress != 0x0970)
    {
        return;
    }

    TAPE_FastSave(regIX, regHL);
